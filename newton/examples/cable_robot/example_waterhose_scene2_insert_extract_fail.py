...
###########################################################################
# Example: Cable Robot Water Hose Insert + Pull/Wiggle
#
# After inserting the cable, the robot releases, re-grasps the cable,
# and shifts sideways to wiggle it. A dormant fixed joint is activated
# at snap with weak angular stiffness so the robot can overcome the
# angular constraint during wiggle.
#
# Command:
#   uv run newton/examples/cable_robot/example_waterhose_scene2_insert_extract_fail.py
#   uv run newton/examples/cable_robot/example_waterhose_scene2_insert_extract_fail.py --primary-view mujoco
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        # so the robot can wiggle the cable. Fixed-k: snap_k_*_max == snap_k_* so
        ...
        # Wiggle motion parameters (translational sideways + downward pull)
        self.wiggle_amplitude = 0.003  # sideways displacement [m]
        self.wiggle_phase_duration = 0.2  # seconds per half-swing
        self.wiggle_count = 15  # number of half-swings
        self.wiggle_pull_z = -0.002  # constant downward pull during wiggle [m]
    ...
    def setup_state_machine(self):
        task_schedule_list = [
            ...
            TaskType.REENGAGE,
            TaskType.REGRASP,
            TaskType.REGRASP_HOLD,
            TaskType.WIGGLE,
            TaskType.FINAL_RELEASE,
            TaskType.FINAL_WITHDRAW,
            TaskType.DONE,
        ]
        ...
        task_time_limits_list = [
            ...
            1.0,  # REGRASP
            1.0,  # REGRASP_HOLD
            999.0,  # WIGGLE (internally timed by phase counter)
            1.0,  # FINAL_RELEASE
            2.0,  # FINAL_WITHDRAW
            999.0,  # DONE
        ]
    ...
    def _compute_regrasp_hold(self):
        """Hold gripper closed at current position."""
        t_elapsed = float(self.sm_task_time_elapsed.numpy()[0]) + self.frame_dt
        wp.copy(self.sm_task_time_elapsed, wp.array([t_elapsed], dtype=float))
        ee_pos, ee_quat = self._get_ee_pose()
        self._write_sm_targets(ee_pos, ee_quat, t=1.0)
    ...
    def _init_wiggle(self):
        """Snapshot EE pose and pick a sideways axis perpendicular to the cable."""
        ee_pos, ee_quat = self._get_ee_pose()
        self._wiggle_center_pos = ee_pos.copy()
        self._wiggle_pull_offset = np.array([0.0, 0.0, self.wiggle_pull_z], dtype=np.float64)
        self._wiggle_ee_quat = ee_quat.copy()
        self._wiggle_side_axis = self._align_axes[0].copy()
        self._wiggle_phase = 0
        if self.verbose:
            print(
                f"  [WIGGLE] sideways: {self.wiggle_count} half-swings, "
                f"amplitude={self.wiggle_amplitude * 1000:.1f}mm, "
                f"pull_z={self.wiggle_pull_z * 1000:.1f}mm, "
                f"period={self.wiggle_phase_duration:.2f}s each"
            )

    def _compute_wiggle(self):
        """Smoothstep sideways: symmetric side-to-side from center.

        Phase 0: center -> +amp
        Phase 1: +amp -> -amp
        Phase 2: -amp -> +amp
        ...
        Last phase returns to center.
        """
        t_elapsed = float(self.sm_task_time_elapsed.numpy()[0]) + self.frame_dt
        wp.copy(self.sm_task_time_elapsed, wp.array([t_elapsed], dtype=float))
        t_lin = min(1.0, t_elapsed / self.wiggle_phase_duration)
        t = t_lin * t_lin * (3.0 - 2.0 * t_lin)

        phase = self._wiggle_phase
        last = self.wiggle_count - 1
        if phase == 0:
            offset = t * self.wiggle_amplitude
        elif phase == last:
            sign = 1.0 if phase % 2 == 0 else -1.0
            offset = sign * self.wiggle_amplitude * (1.0 - t)
        else:
            sign_from = 1.0 if phase % 2 == 0 else -1.0
            sign_to = -sign_from
            offset = (sign_from * (1.0 - t) + sign_to * t) * self.wiggle_amplitude
        target_pos = self._wiggle_center_pos + offset * self._wiggle_side_axis + self._wiggle_pull_offset

        self._write_sm_targets(target_pos, self._wiggle_ee_quat, t=1.0)
        wp.copy(self.sm_gripper_target, wp.array([self.sm_gripper_closed_value], dtype=wp.float32))

    def _advance_wiggle(self, schedule_idx):
        """Advance half-swing counter; move to next task when all done."""
        t_elapsed = float(self.sm_task_time_elapsed.numpy()[0])
        if t_elapsed >= self.wiggle_phase_duration:
            self._wiggle_phase += 1
            if self._wiggle_phase >= self.wiggle_count:
                if self.verbose:
                    print(f"  [WIGGLE] complete ({self._wiggle_phase} half-swings)")
                self._advance_sm_to(schedule_idx + 1)
            else:
                self.sm_task_time_elapsed.zero_()
                body_q_np = self.state_0.body_q.numpy()
                ...
                wp.copy(self.sm_task_init_body_q, wp.array([init_tf], dtype=wp.transform))
                if self.verbose:
                    side = "+" if self._wiggle_phase % 2 == 0 else "-"
                    print(f"  [WIGGLE] half-swing {self._wiggle_phase}/{self.wiggle_count} ({side})")

    def _compute_final_release(self):
        """Open gripper via smoothstep."""
        t_elapsed = float(self.sm_task_time_elapsed.numpy()[0]) + self.frame_dt
        wp.copy(self.sm_task_time_elapsed, wp.array([t_elapsed], dtype=float))
        t_lin = min(1.0, t_elapsed / 1.0)
        t = t_lin * t_lin * (3.0 - 2.0 * t_lin)
    ...
    def _init_final_withdraw(self):
        """Snapshot EE pose for final withdraw."""
        self._final_withdraw_start_pos, self._final_withdraw_quat = self._get_ee_pose()

    def _compute_final_withdraw(self):
        """Move gripper away from cable."""
        t_elapsed = float(self.sm_task_time_elapsed.numpy()[0]) + self.frame_dt
        wp.copy(self.sm_task_time_elapsed, wp.array([t_elapsed], dtype=float))
        t_lin = min(1.0, t_elapsed / 2.0)
        t = t_lin * t_lin * (3.0 - 2.0 * t_lin)
        withdraw_offset = np.array([float(self.sm_withdraw_offset[i]) for i in range(3)], dtype=np.float64)
        target_pos = self._final_withdraw_start_pos + withdraw_offset
        self._write_sm_targets(target_pos, self._final_withdraw_quat, t=t)
        wp.copy(self.sm_gripper_target, wp.array([self.sm_gripper_open_value], dtype=wp.float32))
    ...
    def set_sm_targets(self):
        ...
        elif task == TaskType.REGRASP_HOLD:
            self._compute_regrasp_hold()
        elif task == TaskType.WIGGLE:
            self._compute_wiggle()
        elif task == TaskType.FINAL_RELEASE:
            self._compute_final_release()
        elif task == TaskType.FINAL_WITHDRAW:
            self._compute_final_withdraw()
        else:
            wp.launch(
        ...
        if new_idx != self._sm_prev_task_idx:
            ...
            if new_task == TaskType.WIGGLE:
                self._init_wiggle()

            if new_task == TaskType.FINAL_WITHDRAW:
                self._init_final_withdraw()
