...
class CollisionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # Particle-vs-shape soft contacts for shapes that carry a texture SDF are
        # generated by the cheaper, CUDA-only ``create_soft_contacts_sdf`` kernel
        # (one SDF sample per particle) instead of per-triangle mesh queries.
        # ``create_soft_contacts`` skips these shapes via ``_soft_shape_sdf_index``.
        ...
        if model._shape_sdf_index is not None and wp.get_device(device).is_cuda and shape_count > 0:
            sdf_idx_np = model._shape_sdf_index.numpy()
    ...
    def collide(
        ...
        *,
        ...
        if state.particle_q and model.shape_count > 0:
            ...
            # (skipped by create_soft_contacts above). CUDA-only.
            if self._num_soft_sdf_shapes > 0:
                wp.launch(
