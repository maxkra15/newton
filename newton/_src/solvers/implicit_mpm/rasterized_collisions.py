...
def collision_sdf(
    ...
    if not shared_worlds:
        global_begin = collider.query_world_offsets[0]
        global_count = collider.query_world_offsets[1] - global_begin
        local_begin = collider.query_world_offsets[environment_index + 1]
        local_count = collider.query_world_offsets[environment_index + 2] - local_begin
        query_count = global_count + local_count

    for query_offset in range(query_count):
        stable_collider_id = query_offset
        if not shared_worlds:
            if query_offset < global_count:
                stable_collider_id = collider.query_collider_ids[global_begin + query_offset]
            else:
                stable_collider_id = collider.query_collider_ids[local_begin + query_offset - global_count]
