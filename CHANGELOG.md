...
- Use grouped Warp hash-grid queries for multi-world XPBD and SPH particle neighbors when every particle is world-local and the installed Warp provides the grouped API; otherwise retain the correctness-filtered path, including local/global compatibility.
...
- Fix cross-world XPBD/SPH particle-neighbor, fluid-render, and diffuse-neighbor interactions in colocated worlds. Particles assigned to global world `-1` continue to interact with every world. `SolverXPBD.reorder_particles()` now leaves multi-world models unchanged until segmented per-world reordering is available.
